  ...
  <div class="in-test" v-loading="loading">
    <div class="card-box" v-for="(items, index) in cardBox" :key="index">
      ...
      <el-card shadow="hover" class="box-card">
        ...
        <div class="footer">
          <el-button @click="handleToCreate('view', items.configId, '5')"
            >View</el-button
          >
          <el-button @click="handlePause(items)" v-if="checkApproval !== 'approve'">{{
            items.configStatus===2 ? "Restart" : "Pause"
          }}</el-button>
          <el-button @click="handleStop(items)" v-if="checkApproval !== 'approve'">{{
            items.configStatus===2 ? "Close" : "Stop"
          }}</el-button>
...
export default {
  ...
  methods: {
    ...
    async handlePause(e) {
      this.$confirm(e.configStatus===2?" Are you sure to restart?":"Are you sure to pause?", "Warning", {
        ...
        .then(async () => {
          let form = {
            configId: e.configId,
            status: e.configStatus === 2 ? 1 : 2
          };
          let res = await dashboard.update_process(form);
          this.$message({
            type: "success",
